      ...
      <div className="list-wrapper">
        <div className="list-header">
          ...
          <div className="list-btns">
            <button className="button orange" onClick={handleSortDate}>
              Rušiuoti pagal datą
            </button>
            <button className="button orange" onClick={handleSortRida}>
              Rušiuoti pagal rida (km.)
            </button>
          </div>
        </div>
          ...
          <div className="list-map">
            {newScooter && newScooter.length > 0 ? (
              newScooter.map((item, index) => (
                <div
                  className={`main-list-inside ${
                    item.id === newScooterCreated ? "visiblebg" : ""
                  }${item.id === newScooterUpdated ? "updatedbg" : ""}`}
                  key={index}
                >
              ...
              <span className="loading">Pridėk paspirtuką!</span>
            )}
          </div>
        </div>
      </div>
